...
- **No Host-path write API** — the default carrier lets the browser choose the local destination. An embedding surface may install a native save carrier, but the page receives no general filesystem access or Host path result.
