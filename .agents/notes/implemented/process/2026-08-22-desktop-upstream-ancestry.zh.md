...
已经完成内容接入的 `dsh-v0.1.1-rc.2` 允许一次保持代码树不变的 `-s ours` merge 作为具名例外。rc2 内容仍由既有的[同步决策与验证](../../archived/process/2026-08-21-desktop-sync-dsh-0.1.1-rc.2.md)负责；该例外只把目标补为第二父提交。它既不应用也不验证上游文件，后续目标不得复用。
