...
The already integrated `dsh-v0.1.1-rc.2` target uses one tree-preserving `-s ours` merge as a named exception. The rc2 content remains owned by the existing [synchronization decision and verification](../../archived/process/2026-08-21-desktop-sync-dsh-0.1.1-rc.2.md); the exception only adds the target as a second parent. It neither applies nor verifies upstream files and cannot be reused for a later target.
