...
Desktop 客户端插件在运行时 `inject` 列表中声明 `locale` 和 `slots`，并通过已声明的上下文属性读取它们。Cordis 会在两个服务都存在前保持插件 pending，并在服务提供方消失后重新出现时重新加载插件。各项贡献仍通过 `slots.inject()` 注册，使每个注册跟随其声明 slot 的生命周期，具体规则见 [slot 声明注入](../../../../docs/subsystems/slots.zh.md)。
